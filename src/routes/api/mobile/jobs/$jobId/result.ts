...
import { getTranslationJobResult } from '@/server/jobs/service';
import { logger } from '@/server/logger';

export const Route = createFileRoute('/api/mobile/jobs/$jobId/result')({
  server: {
    handlers: {
      GET: async ({ request, params }) => {
        ...
        try {
          const routeLog = logger.child({
            path: '/api/mobile/jobs/$jobId/result',
          ...
          if (!rateLimit.allowed) {
            routeLog.warn({
              ...
              message: 'Rate limited mobile job result request',
          ...
          const result = await getTranslationJobResult(
            {
              jobId: params.jobId,
            },
            {
