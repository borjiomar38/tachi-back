import { createFileRoute } from '@tanstack/react-router';

import {
  buildApiOkResponse,
  buildHttpRequestContext,
} from '@/server/http/route-utils';
import {
  authenticateAndRateLimitMobileJobRequest,
  buildMobileJobErrorResponse,
  buildMobileJobRateLimitedResponse,
} from '@/server/jobs/http';
import { completeDirectTranslationJobPageUpload } from '@/server/jobs/service';
import { logger } from '@/server/logger';

export const Route = createFileRoute(
  '/api/mobile/jobs/$jobId/pages/$pageNumber/complete'
)({
  server: {
    handlers: {
      POST: async ({ request, params }) => {
        const context = buildHttpRequestContext(request);
        const routeLog = logger.child({
          path: '/api/mobile/jobs/$jobId/pages/$pageNumber/complete',
          requestId: context.requestId,
          scope: 'jobs',
        });

        try {
          const { auth, rateLimit } =
            await authenticateAndRateLimitMobileJobRequest(request, {
              bucket: 'write',
              context,
            });

          if (!rateLimit.allowed) {
            routeLog.warn({
              clientIp: context.clientIp,
              deviceId: auth.device.id,
              jobId: params.jobId,
              licenseId: auth.license.id,
              message: 'Rate limited direct mobile job page upload completion',
              pageNumber: params.pageNumber,
              retryAfterMs: rateLimit.retryAfterMs,
              type: 'rate_limit',
            });

            return buildMobileJobRateLimitedResponse(
              context.requestId,
              rateLimit.retryAfterMs
            );
          }

          const summary = await completeDirectTranslationJobPageUpload(
            {
              jobId: params.jobId,
              pageNumber: params.pageNumber,
            },
            {
              actor: {
                deviceId: auth.device.id,
                licenseId: auth.license.id,
              },
              log: routeLog,
            }
          );

          routeLog.info({
            clientIp: context.clientIp,
            deviceId: auth.device.id,
            jobId: params.jobId,
            licenseId: auth.license.id,
            message: 'Completed direct mobile job page upload',
            pageNumber: params.pageNumber,
            type: 'mutation',
          });

          return buildApiOkResponse(summary, {
            requestId: context.requestId,
          });
        } catch (error) {
          routeLog.error({
            err: error,
            jobId: params.jobId,
            message: 'Direct mobile job page upload completion failed',
            pageNumber: params.pageNumber,
            type: 'mutation',
          });

          return buildMobileJobErrorResponse(error, context.requestId);
        }
      },
    },
  },
});
