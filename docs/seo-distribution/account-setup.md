...
## 2026-05-31 Cycle Setup Advancement

- **Priority advanced:** Official LinkedIn company/founder profile packet
  - **Status:** setup_fields_prepared_owner_action_required
  - **Platform:** LinkedIn company/founder presence
  - **Purpose:** Standardize one verified trust profile for investors, publishers, partner diligence, and startup/community queries without creating or posting a fake presence.
  - **Progress:** Exact fields and exact no-link-first publishing policy were prepared in `docs/seo-distribution/official-launch-profile-packet.md` and this queue row.
  - **Required assets:** Official display name, bio, company logo/avatar, ownership links, support link, media contact, and responsible-use line.
  - **Secret/API reference:** `SEO_AGENT_LINKEDIN_ACCESS_TOKEN`, `SEO_AGENT_LINKEDIN_ORGANIZATION_ID`.
  - **Publish capability after connection:** Agent can prepare official founder/company updates, investor-forward posts, and partnership-ready comments only when owner approval is present and API scope allows.
  - **Next action:** OWNER_ACTION_REQUIRED to create or claim official LinkedIn ownership and upload approved assets.

- **Priority advanced:** Google Search Console and Bing webmaster ownership packet synchronization
  - **Status:** setup_packet_prepared_owner_action_required
  - **Platform:** Search trust infrastructure
  - **Purpose:** Keep owned properties indexed and canonicalized so all outreach can cite source-of-truth pages instead of raw claims.
  - **Progress:** Added explicit queue rows plus non-secret verification/storage references and exact no-automation guardrails in account setup documents.
  - **Required assets:** Current canonical URLs for `https://tachiyomiat.com`, `https://nayovi.com`, and `https://translate-manhwa-ai.com`, sitemap URLs, verification text, and package-signing status policy placeholder.
  - **Secret/API reference:** `SEO_AGENT_GSC_VERIFICATION_METHOD`, `SEO_AGENT_GSC_VERIFICATION_TOKEN`, `SEO_AGENT_BING_VERIFICATION_TOKEN`.
  - **Publish capability after connection:** Agent can prepare owner-approved indexing and coverage summary notes for owner review after credentials are connected.
  - **Next action:** OWNER_ACTION_REQUIRED for property creation/claim, verification, and owner-approved canonical sitemap updates.

### Cycle 2026-05-31: additional setup queue (official accounts)
...
|---|---|---|---|---|---|---|---|---|
| 1 | Google Search Console | Property verification and index baseline for SEO assets | OWNER_ACTION_REQUIRED | Add `https://nayovi.com` and `https://translate-manhwa-ai.com` to GSC, verify via DNS or file upload, submit sitemap for `/ethical-usage/`, `/ocr-privacy/`, and `/api/ocr-pricing/`, and request recrawl for launch pages | Domain ownership proof TXT, canonical URL map, logo and social card screenshots | `GOOGLE_SEARCH_CONSOLE_VERIFICATION_TOKEN`, `TACHI_SEO_DOMAIN_VERIFICATION_CODE` | Track indexing, search impressions, crawl quality, and issue reports for owned trust pages | Complete GSC account delegation and run verification once website ownership credentials are available |
| 2 | Bing Webmaster | Cross-engine crawl visibility and diagnostics | OWNER_ACTION_REQUIRED | Register both SEO domains, upload XML sitemap, verify robots and canonical alignment, optionally enable IndexNow to accelerate updates | Sitemap path, canonical URLs, support/permission statement copy | `BING_VERIFICATION_TOKEN`, `BING_WEBMASTER_CLIENT_ID` | Get Bing coverage diagnostics and recovery workflow for 404/sitelink/duplicate issues | Complete credentials handoff and activate crawl monitoring |
| 3 | YouTube Channel (Official Nayovi) | Publish walkthrough and trust-building videos with policy-safe links | OWNER_ACTION_REQUIRED | Claim/create official channel, publish profile URL to `https://nayovi.com`, upload demo clips + community safety overview, enable channel keywords | 1080p logo lockup, demo script, thumbnail set, intro narration | `YOUTUBE_API_KEY`, `YOUTUBE_CHANNEL_ID` (or manual access fallback) | Release short demo videos and pinned introductory post; provide linkable public proof of workflow and permissions policy | Prepare channel art + first 2 videos before external review |
| 4 | Product Hunt Maker Profile | Increase startup legitimacy and media discoverability | OWNER_ACTION_REQUIRED | Create maker profile with founder details, set canonical website links, create product card with clear non-deceptive description and legal/ethical positioning | Hero image, short tagline, 30-sec demo clip, key feature bullets | `PRODUCT_HUNT_TOKEN`, `PRODUCT_HUNT_MAKER_SLUG` | Run profile updates, launch posts, and founder answers without affiliate spam | Complete Maker profile onboarding and request publish path confirmation |
| 5 | DEV / Medium | Publish long-form technical and legal/ethical trust content | AUTHORIZED_ACCOUNT_REQUIRED | Create or grant Nayovi contributor access; verify email and profile; request posting permissions where restricted | Technical byline copy, architecture diagram, screenshot set | `DEVTO_TOKEN`, `MEDIUM_INTEGRATION_TOKEN` | Publish SEO-friendly technical guides with contextual link mentions and schema-friendly canonical references | Draft content first, then queue for publish review after platform approval |
