...
| high | Search webmaster verification console | Official search ownership for Nayovi, TachiyomiAT, and SEO discovery domains before directory/press outreach scales | setup_packet_refreshed_owner_action_required | Owner verifies domains in Google Search Console and Bing Webmaster Tools manually, chooses DNS/meta/file method, accepts any terms, and stores API access only in approved secret store | Domain list, sitemap URLs, verification method, source-of-truth pages to inspect, public owner status language | `SEO_AGENT_GOOGLE_SEARCH_CONSOLE_CREDENTIALS`, `SEO_AGENT_BING_WEBMASTER_API_KEY`; no verification tokens in docs unless intentionally public | Agent can submit/monitor sitemaps and indexing diagnostics only after official API credentials are connected | Owner verifies `tachiyomiat.com`, `nayovi.com`, and `translate-manhwa-ai.com`; then agent can monitor sitemap/indexing state without guessing. |
| high | Official newsletter/public update archive | Durable owned trust signal for APK review readiness, OCR QA notes, safe-sample testing, and source-of-truth links | owner_review_required | Owner creates/connects official newsletter profile manually, confirms sender identity/compliance settings, and approves first issue before publication | `docs/seo-distribution/newsletter-first-issue-draft.md`, logo/avatar, public contact path, approved screenshots/demo references, package facts if public | `SEO_AGENT_NEWSLETTER_PROFILE_REFERENCE`; platform token only if owner enables official API publishing | Agent can maintain issue drafts and publish only after explicit owner approval and authorized official API workflow | Owner reviews `docs/seo-distribution/newsletter-first-issue-draft.md`, confirms brand-led vs founder-led archive, and keeps contact import/bulk mail disabled. |
