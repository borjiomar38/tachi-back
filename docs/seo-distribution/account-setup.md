...
| Priority | Platform | Purpose | Status | Owner/manual step | Required assets | Secret/API variable or credential reference | Publish capability after connection | Next action |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| high | Google Search Console + Bing Webmaster | Keep crawl/index trust statements honest for launch and distribution evidence | owner_action_required | OWNER_ACTION_REQUIRED: verify `nayovi.com`, `translate-manhwa-ai.com`, and any additional branded host before any crawl claim is published in outbound materials | `docs/seo-distribution/webmaster-and-entity-verification-packet.md`, sitemap URLs, public crawl notes, canonical URL list | `SEO_AGENT_GOOGLE_SEARCH_CONSOLE_CREDENTIALS`, `SEO_AGENT_BING_WEBMASTER_API_KEY` | Owner can run index/audit checks and update internal evidence notes after verification; external materials stay no-link-first until verification status is confirmed | Confirm domains and verify each environment before using index-readiness language in social and partner drafts |
| high | YouTube official channel | Long-form, official support and reviewer evidence channel for policy-safe demos | setup_packet_prepared_owner_action_required | OWNER_ACTION_REQUIRED: claim/connect official channel, approve channel bio, links, and at least one approved demo sequence before any upload or metadata change | `docs/seo-distribution/official-launch-profile-packet.md`, demo policy notes, approved sample policy, logo/avatar, channel branding | `SEO_AGENT_YOUTUBE_REFRESH_TOKEN`, `SEO_AGENT_YOUTUBE_CHANNEL_ID` | Owner-approved channel updates only; no uploads or metadata edits without explicit manual owner approval | Keep YouTube assets in this cycle as draft until owner confirms upload rights and support language |
| high | AppBrain listing readiness profile | Decide whether AppBrain Play-linked featured-listing rules are compatible with Nayovi distribution posture | setup_packet_prepared_owner_action_required | OWNER_ACTION_REQUIRED: confirm AppBrain’s official requirement check and create official source-link fields only if a Play-linked listing lane is valid | `docs/seo-distribution/app-store-source-eligibility-packet.md`, support/pricing URLs, package/screenshot policy, legal-safe trial + token wording | `SEO_AGENT_APPBRAIN_DEVELOPER_REFERENCE` (official account/workspace reference only) | No outbound listing action until compatibility is confirmed with official policy and ownership fields | Keep in gated status; move to review-only draft only if a Play-available lane is confirmed by owner |
...
| Priority | Platform | Purpose | Status | Owner/manual step | Required assets | Secret/API variable or credential reference | Publish capability after connection | Next action |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| high | Google Search Console + Bing Webmaster | Verify crawl and index control across owned trust domains before citing external reach outcomes | owner_action_required | OWNER_ACTION_REQUIRED: confirm ownership and verification for `tachiyomiat.com`, `nayovi.com`, `translate-manhwa-ai.com`; upload/align verification snippets only for policy pages currently approved by owner | `docs/seo-distribution/webmaster-and-entity-verification-packet.md`, XML sitemap links, crawl-control notes, policy and support URLs | `SEO_AGENT_GOOGLE_SEARCH_CONSOLE_CREDENTIALS`, `SEO_AGENT_BING_WEBMASTER_API_KEY` | Owner can run index/status checks and cite non-sensitive verification outcomes in owned pages only | Keep public crawl claims in draft; once credentials are connected, update evidence notes in `docs/seo-distribution/distribution-log.md` |
| high | Official GitHub presence and repository docs | Foundation for technical trust assets and safe maintainer-first community participation on GitHub | ready_for_owner_review | OWNER_ACTION_REQUIRED: confirm canonical `github.com` official namespace, maintainer identity, issue/discussion posting boundaries, and release-doc mapping for external references | `docs/seo-distribution/platform-drafts.md`, OCR checklist page, release summary, support and pricing page URLs | `SEO_AGENT_GITHUB_TOKEN optional; SSH remote is enough for owned repo branches` | Authorized contribution-ready state for owned repositories/docs; external comments only when maintainer policy allows and no link-first posting outside context | Verify `github.com` account identity and merge permission; then prepare first maintainer-first outreach in `docs/seo-distribution/platform-drafts.md` |
| medium | X/Twitter profile and API readiness | Official short-form trust updates for OCR workflow verification and release signals | setup_packet_prepared_owner_action_required | OWNER_ACTION_REQUIRED: create or claim official handle manually, approve anti-spam policy, and define no-link-first support posting pattern | `docs/seo-distribution/open-social-profile-packet.md`, brand avatar/banner, launch timing policy, concise no-link templates | `SEO_AGENT_X_ACCESS_TOKEN` | Optional API-assisted publishing only after owner approves compliant content policy | Prepare post ladder in `docs/seo-distribution/platform-drafts.md`; mark `AUTHORIZED_ACCOUNT_REQUIRED` until account/token exists |

### 2026-06-01 Cycle Advancement (new)

### 2026-06-01 Official trust-control setup packet additions

| Priority | Platform | Purpose | Status | Owner/manual step | Required assets | Secret/API variable or credential reference | Publish capability after connection | Next action |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| high | Crunchbase startup profile | Investor and trust reference for founder/partner conversations before launch or revenue messaging | setup_packet_prepared_owner_action_required | OWNER_ACTION_REQUIRED: claim official profile, confirm company identity fields, choose public metrics policy, and approve disclosure scope | `docs/seo-distribution/official-launch-profile-packet.md`, startup profile fields, support/pricing URLs, responsible-use statement | `SEO_AGENT_CRUNCHBASE_PROFILE_REFERENCE` | Owner can cite the profile as a consistency source only after metrics and entity fields are approved | Keep draft state until owner confirms investor-facing public fields and media policy |
| medium | Bluesky and Mastodon open-social stream | Trust-first official short updates and discovery of approved, non-promotional trust angles | setup_packet_prepared_owner_action_required | OWNER_ACTION_REQUIRED: create/claim official instance/handle path manually, publish official profile fields and no-link-first policy | `docs/seo-distribution/open-social-profile-packet.md`, official profile copy, avatar/banner, canonical links, no-link-first post templates | `SEO_AGENT_BLUESKY_HANDLE`, `SEO_AGENT_BLUESKY_APP_PASSWORD`, `SEO_AGENT_MASTODON_INSTANCE_URL`, `SEO_AGENT_MASTODON_ACCESS_TOKEN` | Authorized posting only after owner approval; no bulk comments, follows, or repost automation | Prepare exact posting packet once one official handle is approved; keep targets AUTHORIZED_ACCOUNT_REQUIRED |

| Priority | Platform | Purpose | Status | Owner/manual step | Required assets | Secret/API variable or credential reference | Publish capability after connection | Next action |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| medium | Google Search Console ownership + Android verification posture | Strengthen crawl confidence for owned pages before third-party trust claims and directory mentions | setup_packet_prepared_owner_action_required | OWNER_ACTION_REQUIRED: confirm whether Google’s phased Android developer verification and app-safety notices apply to non-Play installations; capture official verification wording for owned pages only after owner confirms package facts | `docs/seo-distribution/webmaster-and-entity-verification-packet.md`, official verification page notes, support and policy copy, source-of-truth URL ladder | `SEO_AGENT_GOOGLE_SEARCH_CONSOLE_CREDENTIALS`, `SEO_AGENT_GOOGLE_PLAY_CONSOLE_REFERENCE` | Owner can verify index/readiness and share non-sensitive crawl notes only once package and policy language are approved | Add this verification language into trust assets before any AppBrain/XDA/AppBrain-like external mention, then keep drafts no-link-first until profile identity is approved |
| high | Android app distribution submission readiness (directory/app directory submitter) | Pre-wire fields for app discovery portals that require claimed developer identity and policy statements | setup_packet_prepared_owner_action_required | OWNER_ACTION_REQUIRED: create official package fact sheet (APK source URL, pricing model, support contact, screenshot policy), and define where source-link-only submissions are allowed | Official APK/package packet, source URL, screenshot/permission policy, package/signing notes (owner-approved), support contact path | `SEO_AGENT_ANDROID_PACKAGE_REFERENCE`, `SEO_AGENT_ANDROID_REVIEW_PORTAL_REFERENCE` | Agent can prepare no-link-first fit packets and conditional listing rows, then move to link-ready only after owner confirms package/signing fields | Prepare one source-safe submission template now; publish only when official account/portal terms allow |
...
## 2026-06-01 Official Account Setup Continuation (new this cycle)

| Priority | Platform | Purpose | Status | Owner/manual step | Required assets | Secret/API variable or credential reference | Publish capability after connection | Next action |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| high | LinkedIn company + founder pages | Founder/investor trust and partner credibility before paid social outreach | setup_packet_prepared_owner_action_required | OWNER_ACTION_REQUIRED: create or claim official company page, link founder profile role/title, approve anti-promo posting policy, and lock canonical link set | `docs/seo-distribution/official-launch-profile-packet.md`, logo/avatar, mission one-liner, canonical URL set, responsible-use line | `SEO_AGENT_LINKEDIN_ACCESS_TOKEN`, `SEO_AGENT_LINKEDIN_ORGANIZATION_ID` | Manual or token-enabled profile posts (value-first, no upvote manipulation) after policy confirmation | Owner confirms account ownership and signs off on public fields |
| high | Reddit official community support profile | Rules-safe technical support in Android and translator communities | setup_packet_prepared_owner_action_required | OWNER_ACTION_REQUIRED: create/claim profile manually, complete subreddit rule matrix, approve no-link-first template and escalation policy | `docs/seo-distribution/platform-drafts.md`, moderation matrix, no-link-first templates, escalation notes | `SEO_AGENT_REDDIT_CLIENT_ID`, `SEO_AGENT_REDDIT_CLIENT_SECRET`, `SEO_AGENT_REDDIT_REFRESH_TOKEN` | Authorized owner-approved comment/answer drafts only; no automated participation or spam patterns | Owner confirms official profile + moderation boundaries before any community posting |
...
| Priority | Platform | Purpose | Status | Owner/manual step | Required assets | Secret/API variable or credential reference | Publish capability after connection | Next action |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| high | Google Search Console + Bing Webmaster | Owned domain verification and crawl governance before trusted directory/editorial mentions | owner_action_required | OWNER_ACTION_REQUIRED: claim and verify `tachiyomiat.com`, `nayovi.com`, and `translate-manhwa-ai.com` in official accounts; approve crawl/sitemap language | `docs/seo-distribution/webmaster-and-entity-verification-packet.md`, sitemap URLs, canonical index list, no-mirror verification wording | `SEO_AGENT_GOOGLE_SEARCH_CONSOLE_CREDENTIALS`, `SEO_AGENT_BING_WEBMASTER_API_KEY` | Index status checks and crawl issue summaries after owner verification and credential connection | OWNER_ACTION_REQUIRED: complete identity and property verification; keep public pages on pending-language until official verification wording is approved |
| medium | YouTube official channel | Durable demo and review-surface with responsible-use proof for Android OCR support flow | setup_packet_prepared_owner_action_required | OWNER_ACTION_REQUIRED: create/claim the official channel, approve one approved-sample demo, then approve asset and posting policy before any manual/video action | `docs/seo-distribution/account-setup.md`, approved demo asset list, video description template, canonical links, reviewer-safe screenshots | `SEO_AGENT_YOUTUBE_REFRESH_TOKEN` | Manual upload/description updates after optional API connection; no automated posting | Owner approves demo timing, thumbnails, and no-mirror policy; only publish if direct demo use case is approved |
| high | Crunchbase / startup profile | Startup/partner trust surface for diligence and investor review | setup_packet_prepared_owner_action_required | OWNER_ACTION_REQUIRED: create or claim official profile with founder/company truth fields and approved public-contact policy | `docs/seo-distribution/official-launch-profile-packet.md`, official company bios, support/contact pages, canonical links | `SEO_AGENT_CRUNCHBASE_PROFILE_REFERENCE` | Public consistency layer for diligence conversations and launch/community mentions | Owner approves official fields; no metric publication or paid placement without explicit authorization |
...
| high | LinkedIn company/founder profile | Founder/company trust, partner/investor visibility | setup_fields_prepared_owner_action_required | OWNER_ACTION_REQUIRED: owner creates or claims the official LinkedIn company or founder page and finalizes copy before any API publish. Prepare exact fields: Brand name, short bio with no chapter-hosting note, public website links (`https://nayovi.com`, `https://translate-manhwa-ai.com`, `https://tachiyomiat.com`), support URL, media contact URL/email, logo 1024x1024, and banner image. | Logo, banner, company bio, website links, media contact path, responsible-use line, no-mirror language, trial-to-paid decision note | SEO_AGENT_LINKEDIN_ACCESS_TOKEN, SEO_AGENT_LINKEDIN_ORGANIZATION_ID | Publish manual LinkedIn company/founder updates and partnership-first visibility posts after owner signs each field; post only after official profile is verified as Nayovi-owned | OWNER_ACTION_REQUIRED: create/claim page, approve first post, upload approved assets, and enable manual-only mode until API token use is explicitly approved. |
| medium | Product Hunt maker/company | Launch credibility and early product feedback | setup_fields_prepared_owner_action_required | OWNER_ACTION_REQUIRED: owner creates or connects official maker profile and manually accepts launch terms before any launch draft is used. Prepare exact fields: public product title/slug, one-sentence tagline, short problem statement, trial-to-paid context, support URL, responsible-use line, screenshot policy, and no-upvote-request draft policy. | Logo, tagline, demo video, product screenshots, pricing/support links, maker comment | SEO_AGENT_PRODUCTHUNT_TOKEN | Prepare launch page and authorized maker comments | Owner chooses launch timing, verifies launch wording against source-of-truth pages, uploads approved media, and approves a no-upvote-request first-post policy; no launch actions before this approval. |
| medium | DEV/Medium technical publishing | Technical authority for OCR, merge QA, hosted Android workflow | setup_fields_prepared_owner_action_required | OWNER_ACTION_REQUIRED: owner creates or connects official developer publication profile and approves canonical policy. Prepare exact fields: profile/bio text, byline style, official source URLs, sample-safe screenshot policy, and article-to-product relationship paragraph. | Canonical article drafts, profile bio, logo, approved screenshots | SEO_AGENT_DEVTO_API_KEY, SEO_AGENT_MEDIUM_INTEGRATION_TOKEN | Publish canonical technical articles where allowed | Owner creates/connects official DEV or Medium profile, approves first article copy, enables manual-only publish by default, and permits API-driven publish only after first manual validation. |
...
| high | Direct APK source URL profile | Official source-first profile for app-source tools, Android newsletters, app directories, and reviewers that need a verified URL instead of a mirror | setup_packet_prepared_owner_action_required | Owner confirms package name, APK hash, signing fingerprint, Android developer verification status, source-link policy, and approved screenshot/demo references | `docs/seo-distribution/direct-apk-source-url-packet.md`, official download URL, support/pricing/privacy/terms links, no-mirror language | `SEO_AGENT_APK_DIRECTORY_PROFILE_REFERENCE`, `SEO_AGENT_ANDROID_DEVELOPER_CONSOLE_REFERENCE`, `SEO_AGENT_SCREENSHOT_ASSET_REFERENCE` | Agent can prepare source-first citation notes after owner approval; no external GitHub issue, directory submission, APK upload, mirror permission, or app-source listing action automatically | Owner confirms public package facts and whether direct-APK tooling or Android directory surfaces may cite the official source URL. |
| medium | Official Discord/community support rules | Truthful support/community surface for beta testers, reviewers, and approved-sample pilot questions without unofficial DMs or chapter sharing | setup_packet_prepared_owner_action_required | Owner decides whether Discord or another community belongs in the trust stack, creates it manually if desired, accepts terms, approves moderation rules, and chooses invite policy | Official profile copy, support URL, no-piracy rules, review-code escalation policy, approved-sample rules, moderator identity | `SEO_AGENT_COMMUNITY_PROFILE_REFERENCE`, `SEO_AGENT_DISCORD_BOT_TOKEN` only if owner later authorizes official bot/API workflow | Agent can draft rules, welcome copy, and support triage posts after owner approval; no server creation, invite publication, moderation action, or automated posting without authorization | Owner decides if a community is needed before public invites, approves rules, and confirms no chapter uploads, private-code leaks, or unofficial support DMs. |
...
## 2026-05-31 Cycle Setup Continuation (owned tasks prioritized before backlink execution)

### Setup tasks advanced in this cycle

| Priority | Platform | Purpose | Status | Owner/manual step | Required assets | Secret/API variable or credential reference | Publish capability after connection | Next action |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| high | LinkedIn company/founder page | Official trust surface for partner, investor, and partner diligence contexts | OWNER_ACTION_REQUIRED | OWNER_ACTION_REQUIRED: create or claim the official owner account, approve legal/brand identity fields, and finalize no-link-first trust policy | Logo/avatar, concise mission copy, canonical links (`https://nayovi.com`, `https://tachiyomiat.com`, `https://translate-manhwa-ai.com`), support/contact path, publication cadence | `SEO_AGENT_LINKEDIN_ACCESS_TOKEN`, `SEO_AGENT_LINKEDIN_ORGANIZATION_ID` | Enable profile metadata and future official updates once ownership and token scope are confirmed. | Complete account/identity ownership step first, then keep draft-only updates until launch cadence and policy are approved. |
| high | Product Hunt maker profile | Founder-first launch validation for a transparent, anti-spam community signal | OWNER_ACTION_REQUIRED | OWNER_ACTION_REQUIRED: create/finalize official maker identity under a personal account, confirm no company/account spam behavior, and approve launch timing and comments policy | Maker-ready copy, launch checklist, canonical landing links, launch tags, optional screenshots | `SEO_AGENT_PRODUCTHUNT_TOKEN` | Enable launch draft preparation and manual launch workflow when account + timeline are approved | Confirm access rules and launch readiness with owner before any post or launch comment is prepared publicly. |

## Cycle 2026-05-31 — Priority account bootstrap (official ownership and API readiness)

| Priority | Platform | Purpose | Status | Owner / Manual step | Required assets | Secret/API ref | Publish capability after connection | Next action |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| P1 | LinkedIn (Company + Founder profile) | Establish high-trust founder and brand presence for partnerships, investor updates, and outbound social proof | OWNER_ACTION_REQUIRED: Manual account creation/phone verification and owner identity confirmation are still required before API automation can proceed | Human owner creates official LinkedIn company page `Nayovi` and founder profile references, sets role/title, adds official website and privacy policy links, uploads logo (512x512) and banner (1500x500), and prepares 80-char mission + 2-post editorial schedule | Branded logo, banner image, short company description, founder bio, official URLs, governance statement, support contact email | `SEO_AGENT_LINKEDIN_ACCESS_TOKEN` stored in `/opt/tachi-back/.env.seo-distribution-agent` (for future API-driven posting only) | Post on company page, run lightweight founder/employee network outreach, send partner introduction messages, publish trust-first milestones | Add `docs/locales` copy for LinkedIn-ready claim lines, then prepare AUTHORIZED_ACCOUNT_REQUIRED draft for founder announcement |
| P1 | Product Hunt (Maker/Company account) | Add launch/release credibility signal and launch-channel visibility for build updates | OWNER_ACTION_REQUIRED: Creator account setup + phone/ownership verification + optional public API token onboarding is blocked outside this cycle | Human owner creates official Product Hunt maker account linked to nayovi.com, verifies profile ownership, enables email forwarding, and prepares launch post package with screenshot and product statement | Product logo/icon, app screenshots (3), privacy-safe value statement, pricing plan summary, support link, brand website URL | `SEO_AGENT_PRODUCTHUNT_ACCESS_TOKEN` stored in `/opt/tachi-back/.env.seo-distribution-agent` if API flow is enabled | Publish launch pages, participate in maker launch cycles, and place launch post links in distribution packets | Owner to complete Product Hunt setup and share maker token scope; then prepare a launch draft in `platform-drafts.md` |
| P2 | YouTube channel | Build trust through feature demonstration and creator-first SEO video asset | OWNER_ACTION_REQUIRED: Channel and brand verification cannot be automated; no channel configured yet | Human owner creates official channel under brand email, verifies ownership, uploads brand intro short + workflow demo script, sets channel description with canonical links and support path | Branding kit, 1 demo storyboard, voiceover text, canonical homepage/support links, contact email | `SEO_AGENT_YOUTUBE_API_KEY` or OAuth credentials in `/opt/tachi-back/.env.seo-distribution-agent` (manual consent flow if/when OAuth is added) | Publish demo video, shorts, and creator updates; embed videos on website as trust assets | Prepare owned script + thumbnail outline now and mark for human execution when channel is live |

## 2026-06-01 Cycle Setup Advancement

### 2026-06-01 Priority setup rows

| Priority | Platform | Purpose | Status | Owner/manual step | Required assets | Secret/API variable or credential reference | Publish capability after connection | Next action |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| high | LinkedIn company profile | Founder/investor trust surface + official company narrative with no-spam posture | setup_packet_prepared_owner_action_required | OWNER_ACTION_REQUIRED: create/claim official company page manually, connect founder page with role disclosure, then approve canonical bio/website link set and “no chapter-hosting” statement | `docs/seo-distribution/account-setup.md` canonical block, logo/avatar, company one-liner, support URL, anti-spam defaults, responsible-use policy | `SEO_AGENT_LINKEDIN_ACCESS_TOKEN`, `SEO_AGENT_LINKEDIN_ORGANIZATION_ID` | Agent can prepare company posts, profile refresh suggestions, and draft press/partnership summaries only after owner review + authorized API session; no auto-posting | Owner confirms official account ownership and freezes posting guardrails; keep as draft until authorization is explicit |
| high | Crunchbase + startup profile packet | Investor and partner diligence identity surface plus cross-site consistency (with no hidden metrics) | owner_action_required | OWNER_ACTION_REQUIRED: create or claim official profile manually and align official company description, HQ status, contact path, and legal/mission wording before publishing any link | `docs/seo-distribution/account-setup.md` startup identity template, logo/banner, official support/terms/privacy pages, media-contact reference, no-mirror policy text | `SEO_AGENT_CRUNCHBASE_PROFILE_REFERENCE` | After owner approval, agent can sync factual fields for consistency and maintain a non-promotional profile reference block for outreach reports | Publish only after founder/company owner review and manual confirmation of all public facts |

## 2026-06-01 Setup Advancement Addendum

| Priority | Platform | Purpose | Status | Owner/manual step | Required assets | Secret/API variable or credential reference | Publish capability after connection | Next action |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| P0 | Google Search Console + Bing Webmaster | Strengthen crawl/index trust and prove canonical consistency for high-intent landing pages | setup_packet_prepared_owner_action_required | OWNER_ACTION_REQUIRED: configure both platforms manually with domain verification first, then add sitemap inventory for `tachiyomiat.com`, `nayovi.com`, and `translate-manhwa-ai.com` | DNS ownership evidence, brand identity assets, canonical URL map, sitemap list, privacy/support links | `SEO_AGENT_GSC_CREDENTIAL_PATH`, `SEO_AGENT_BING_VERIFICATION_RECORD` | Enable index-monitoring, crawl coverage reports, URL submission, and backlink-acceptance readiness checks once ownership is confirmed | Owner performs domain verification and shares confirmation; then agent prepares index health and coverage summary drafts |
| P1 | YouTube official brand channel | Publish accessible product walkthroughs and establish a creator-facing trust artifact for partnerships/investor diligence | setup_packet_prepared_owner_action_required | OWNER_ACTION_REQUIRED: create/claim brand channel, verify ownership, add About links, upload 1 non-promotional workflow video, and pin source links | Brand logo/banner, short demo script, ownership disclaimers for safe usage examples, official URLs for channel bio | `SEO_AGENT_YOUTUBE_CHANNEL_ID`, `SEO_AGENT_YOUTUBE_API_KEY` | Agent can prepare upload checklist, episode briefs, subtitle-safe title templates, and embed-ready metadata packets after ownership and channel policy checks | Owner to finish official channel claim and upload first demo; then open optional API integration gate for scheduled releases |

## 2026-06-01 Setup Priority Status Snapshot

- Advanced tasks this cycle: `Google Search Console + Bing Webmaster` and `YouTube official brand channel` setup packets prepared in docs, both marked `OWNER_ACTION_REQUIRED` and blocked from autonomous posting until owner action completes.
- Published gating rule: no external posting, no directory submission, and no tokenized API actions without explicit manual owner confirmation in this cycle.

## 2026-06-01 Cycle Advancement

| Priority | Platform | Purpose | Status | Owner/manual step | Required assets | Secret/API variable or credential reference | Publish capability after connection | Next action |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| high | LinkedIn official company/founder profile | Build a trustworthy identity for press, partners, and enterprise conversations | setup_packet_prepared_owner_action_required | OWNER_ACTION_REQUIRED: create or claim official company and/or founder page manually (no automated signup). Approve bio/canonical links and manual posting policy first. Keep a process-first tone and no upvote/lead-push language. | `docs/seo-distribution/official-launch-profile-packet.md`, `docs/seo-distribution/official-profile-readiness-checklist.md`, logo/avatar, short bio, canonical link order, responsible-use line | `SEO_AGENT_LINKEDIN_ACCESS_TOKEN`, `SEO_AGENT_LINKEDIN_ORGANIZATION_ID` | After ownership and policy approval, publish owner-reviewed company/founder updates only through manual/API flow. | Await owner identity proof, approved copy, and no-link-first moderation rules; do not publish until account setup is confirmed |
| high | YouTube official channel | Add durable product videos and walkthrough credibility for OCR workflow trust and support transparency | owner_action_required | OWNER_ACTION_REQUIRED: create/connect official channel manually; approve one approved-sample demo, thumbnail policy, and legal-safe script. No videos uploaded until manual owner approval. | `docs/seo-distribution/account-setup.md`, approved demo asset list, channel bio, canonical links, screenshot policy | `SEO_AGENT_YOUTUBE_REFRESH_TOKEN` | Upload/metadata updates and playlist creation after official channel confirmation and channel owner scope approval. | Owner approval needed for channel identity, content policy, and whether any media can be published in draft mode |
| high | Product Hunt maker/company profile | Launch + high-intent build-in-public channel for structured, non-manipulative feedback | setup_packet_prepared_owner_action_required | OWNER_ACTION_REQUIRED: create or connect official maker/company profile manually, confirm official launch timing, and enforce no-upvote/anti-engagement posture. Keep a launch-safe first-post and maker profile fields checklist only. | `docs/seo-distribution/official-launch-profile-packet.md`, launch-safe post draft, logo/banner, demo media, official pricing/support URLs | `SEO_AGENT_PRODUCTHUNT_TOKEN`, `SEO_AGENT_PRODUCTHUNT_PROFILE_REFERENCE` | After manual approval, prepare draft maker updates and optional maker comment from official profile only (no spam mechanics). | Owner reviews launch timing, copy, and channel rules before any external readiness handoff |

## 2026-06-01 Priority Setup Continuation

### 2026-06-01 Owner-gated account packets advanced

| Priority | Platform | Purpose | Status | Owner/manual step | Required assets | Secret/API variable or credential reference | Publish capability after connection | Next action |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| high | LinkedIn founder profile | Build founder trust before launch communities, media pitches, and investor reviews | setup_packet_prepared_owner_action_required | OWNER_ACTION_REQUIRED: create or claim an official founder profile, approve role-title language, and confirm allowed posting frequency and no-link-first rules | Founder role bio, publication cadence, canonical links, anti-spam policy statement | `SEO_AGENT_LINKEDIN_ACCESS_TOKEN`, `SEO_AGENT_LINKEDIN_ORGANIZATION_ID` | Publish profile metadata and owner-approved thought posts only from official account once identity and posting policy are confirmed | Owner confirms profile ownership and communication policy; keep all drafts as `AUTHORIZED_ACCOUNT_REQUIRED` until then |
| medium | Mangasplaining creator media path | Open creator-localization media lane for approved-sample QA and translation quality discussions | setup_packet_prepared_owner_action_required | OWNER_ACTION_REQUIRED: confirm who is the public contact and whether founder/company byline is ready; keep content topic-first with no paid promotion | One invited topic note, approved-sample policy language, concise founder byline, optional demo/asset reference | `SEO_AGENT_MANGASPLAINING_CONTACT_REFERENCE` | Use as a watch/draft channel first; post only from owner-approved manual outreach | Keep as `OWNER_REVIEW_REQUIRED` and only move to draft-ready after channel-level permission and topic fit review |

## 2026-06-01 Cycle Setup Advancement (new)

| Priority | Platform | Purpose | Status | Owner/manual step | Required assets | Secret/API variable or credential reference | Publish capability after connection | Next action |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| high | LinkedIn official company/founder | Builder/investor trust and official profile governance for long-form updates | ready_for_owner_review | OWNER_ACTION_REQUIRED: confirm official company + founder ownership; approve bio copy, official role titles, canonical links, and posting frequency policy. No launch hype. | `docs/seo-distribution/official-launch-profile-packet.md`, `docs/seo-distribution/official-profile-readiness-checklist.md`, logo/avatar, city/country policy fields, support URL set, no-mirror policy line | `SEO_AGENT_LINKEDIN_ACCESS_TOKEN`, `SEO_AGENT_LINKEDIN_ORGANIZATION_ID` | Owner-approved only: profile metadata updates and value-first founder updates from official account | Run profile copy review once official ownership is confirmed; keep current drafts as `OWNER_REVIEW_REQUIRED` until then |
| high | Product Hunt maker/company profile | Build public launch/proof surface for builder and support communities | ready_for_owner_review | OWNER_ACTION_REQUIRED: create or claim official maker profile manually; set launch policy, anti-spam posture, and official links before any draft is published | `docs/seo-distribution/official-launch-profile-packet.md`, logo/avatar, launch policy, support/faq links, short no-link founder blurb | `SEO_AGENT_PRODUCTHUNT_TOKEN`, `SEO_AGENT_PRODUCTHUNT_PROFILE_REFERENCE` | After account + profile verification, use for manual maker updates and launch note publication | Move related draft packet in `docs/seo-distribution/platform-drafts.md` to owner-ready once policy review passes |

### 2026-06-01 Cycle Advancement — Owned account setup packet progression

| Priority | Platform | Purpose | Status | Owner/manual step | Required assets | Secret/API variable or credential reference | Publish capability after connection | Next action |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| high | Google Search Console + Bing Webmaster | Move from ticketed planning to review-ready verification packet before external trust claims | ready_for_owner_review | OWNER_ACTION_REQUIRED: confirm all property ownership and upload verified ownership snippets for `nayovi.com` and `translate-manhwa-ai.com`; keep crawl language non-sensitive until owner approves | `docs/seo-distribution/webmaster-and-entity-verification-packet.md`, sitemap URLs, support/pricing policy URLs, ownership notes per property | `SEO_AGENT_GOOGLE_SEARCH_CONSOLE_CREDENTIALS`, `SEO_AGENT_BING_WEBMASTER_API_KEY` | Owner can run verified index-readiness checks and approve only non-sensitive evidence references in owned content | Add property-specific verification evidence section to `docs/seo-distribution/distribution-log.md`; only then expose crawl-readiness mention in non-promotional assets |
| high | LinkedIn company + founder pages | Build founder/investor trust surface with explicit ownership and no-self-promo posture | ready_for_owner_review | OWNER_ACTION_REQUIRED: claim official LinkedIn company page and founder page, map role/title, and set canonical URL fields and anti-spam posting policy | `docs/seo-distribution/official-launch-profile-packet.md`, logo/avatar, mission one-liner, responsibility-safe summary, support/contact path | `SEO_AGENT_LINKEDIN_ACCESS_TOKEN`, `SEO_AGENT_LINKEDIN_ORGANIZATION_ID` | Manual posting and optional token-assisted posting after approval; policy-only value content (release notes, user process guidance, no paid placement claims) | Keep drafts in `docs/seo-distribution/platform-drafts.md` as `AUTHORIZED_ACCOUNT_REQUIRED` until account is claimed |
| medium | GitHub official org/repo docs | Finalize trusted technical trust surface and documentation-first contribution path | ready_for_owner_review | OWNER_ACTION_REQUIRED: confirm official ownership details for org/repo and allow maintainer-safe docs updates in owned repository channels | `docs/seo-distribution/official-github-trust-packet.md`, release notes schedule, support/FAQ links, OCR + responsible-use boundaries | `SEO_AGENT_GITHUB_TOKEN optional; SSH remote is enough for owned repo branches` | Owner can publish neutral, technical trust assets and process docs; outside repository activity only with invited/community context | Mark external community contributions as `AUTHORIZED_ACCOUNT_REQUIRED` unless invited and value-first |

## 2026-06-02 Cycle Account Setup Progress

| Priority | Platform | Purpose | Status | Owner/manual step | Required assets | Secret/API variable or credential reference | Publish capability after connection | Next action |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| high | YouTube official channel and Shorts profile | Build transparent visual trust artifacts for installer walkthrough, release notes, and responsible-use explanation | owner_action_required | OWNER_ACTION_REQUIRED: create/claim official company YouTube channel, confirm ownership of `Nayovi`, set About + links, and create one no-advertorial demo/caution video draft approved by owner | `docs/seo-distribution/official-media-kit.md`, brand logo, intro/outro card, `https://nayovi.com/download`, `https://nayovi.com/support`, screenshot/permission-safe demo policy | `SEO_AGENT_YOUTUBE_REFRESH_TOKEN`, `SEO_AGENT_YOUTUBE_CHANNEL_ID` (reference) | Manual publishing only for product-safe explainers after owner approval of tone, no upvote/manipulation policy, and safe video links | Build one official playlist before any external posting claims. Confirm if short-form video is needed by growth owner. |
| high | Google Search Console + Bing Webmaster API readiness | Crawl and index readiness for owned trust pages before using external citations | owner_action_required | OWNER_ACTION_REQUIRED: confirm ownership or verification pathway for `nayovi.com`, `tachiyomiat.com`, and `translate-manhwa-ai.com`, then approve verified indexing language for public reports | `docs/seo-distribution/webmaster-and-entity-verification-packet.md`, XML sitemap links, canonical policy update checklist | `SEO_AGENT_GOOGLE_SEARCH_CONSOLE_CREDENTIALS`, `SEO_AGENT_BING_WEBMASTER_API_KEY` | Index and crawl visibility status can be monitored only after verification and policy-safe wording is owner-approved | Add verification progress note in `docs/seo-distribution/distribution-log.md`; keep any public crawl claims draft-only until ownership is visible.

## Cycle addendum · 2026-06-02

### Account setup tasks (prioritized for manual owner completion)

| priority | platform | purpose | status | owner/manual step | required assets | secret/API reference | publish capability after connection | next action |
| --- | --- | --- | --- | --- | --- | --- | --- |
| critical | Google Search Console (nayovi.com & translate-manhwa-ai.com) | Verify ownership and secure index/coverage signals for official domains | in_progress | OWNER_ACTION_REQUIRED: create DNS TXT records for both domains from Search Console admin UI and confirm site ownership by adding the exact snippets to DNS. Keep only final verification code in `/opt/tachi-back-distribution-agent/.env.seo-distribution-agent`-style secret store. | Domain ownership proofs (DNS TXT access), canonical sitemap, `robots.txt`, brand logo 512x512, brand favicon | `SEO_AGENT_GSC_NAYOVI_VERIFICATION`, `SEO_AGENT_GSC_TRANSLATE_MAI_VERIFICATION` (reference-only; store values in `.../.env.seo-distribution-agent`) | Submit both properties, request indexing for existing guides and support pages, enable performance + coverage exports for monthly checks | Add verification step in owner action queue and validate once both TXT records propagate |
| high | LinkedIn Company Page (Nayovi) | Establish founder/developer trust signal in creator and investor networks | blocked | OWNER_ACTION_REQUIRED: log in with official business owner, create/verify Company Page with public URL, then request claimed page from profile. Canonical links must point to https://nayovi.com, with CTA to https://nayovi.com/download and no reference to legacy branding. | Logo (square + landscape), cover image 1128x191, verified website URL, short mission statement, founder bio, support/legal links | `SEO_AGENT_LINKEDIN_ACCESS_TOKEN`, `SEO_AGENT_LINKEDIN_PAGE_ID` | Publish official product updates, thought-leadership posts, and hiring/partnership updates; attach canonical links in About and Featured sections | Save asset pack and page screenshots for `docs/seo-distribution/platform-drafts.md` and set status to ready after owner confirms account ownership |
| medium | Product Hunt Maker Hub | Launch listing + visibility for launch season + inbound startup signals | blocked | OWNER_ACTION_REQUIRED: create/confirm maker account and claim product hub; if already exists, request page claim for nayovi.com primary listing and add launch details with permissions-safe positioning. | Product title, short description (<80 chars), launch screenshot (no app UI), founder name, primary URL https://nayovi.com, social links, category recommendation (productivity), short demo video, privacy statement | `SEO_AGENT_PRODUCT_HUNT_TOKEN`, `SEO_AGENT_PRODUCT_HUNT_MAKER_EMAIL` | Create a public launch page and maintain launch schedule, hunter requests, and follow-up comments with community updates | Prepare draft launch copy and assets now; move to owner_review with manual API token once account is connected |
| medium | YouTube (Nayovi channel) | Host long-form walkthroughs and trust-first creator content without APK-promotional claims | blocked | OWNER_ACTION_REQUIRED: create/verify official channel under Nayovi brand, upload two short videos (permission-safe walkthrough + creator workflow), set channel links to https://nayovi.com and verify claimed website domain | Channel brand kit, intro/outro assets, two original non-app teaser intros, link-only CTA to https://nayovi.com/download in description | `SEO_AGENT_YOUTUBE_API_KEY`, `SEO_AGENT_YOUTUBE_CHANNEL_ID` | Publish videos, playlist for feature comparisons, and cross-link support/guide pages from description | Add content plan and first two scripts to `docs/seo-distribution/content-calendar.md` after account is active |


## 2026-06-02 Cycle Setup Advancement (owner-action packets)

| Priority | Platform | Purpose | Status | Owner/manual step | Required assets | Secret/API variable or credential reference | Publish capability after connection | Next action |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| high | Google Search Console + Bing Webmaster + crawl readiness | Finalized official ownership gate before any backlink or directory mention of index status | ready_for_owner_review | OWNER_ACTION_REQUIRED: verify `tachiyomiat.com`, `nayovi.com`, and `translate-manhwa-ai.com` in both consoles, confirm allowed public crawl claims, and align sitemap freshness with the trust pages | `docs/seo-distribution/webmaster-and-entity-verification-packet.md`, XML sitemap URLs, crawl-control notes, supported claim ladder, ownership proof checklist | `SEO_AGENT_GOOGLE_SEARCH_CONSOLE_CREDENTIALS`, `SEO_AGENT_BING_WEBMASTER_API_KEY` | Owner can publish only crawl-evidence-safe statements and stop-light status in owned docs and profile bios after verification | Keep all index-crawl wording in draft until owner approves package/signing status and verification notes |
| medium | YouTube short-form demo readiness | Build a durable creator-facing trust channel for process-first OCR workflow guidance and onboarding transparency | setup_packet_prepared_owner_action_required | OWNER_ACTION_REQUIRED: create/confirm official channel, add official profile text, upload assets, and confirm no-review or mirror-first upload policy before posting any content | `docs/seo-distribution/official-launch-profile-packet.md`, approved thumbnail/mockups, script-safe demo outline, channel description, support and pricing links | `SEO_AGENT_YOUTUBE_REFRESH_TOKEN`, `SEO_AGENT_YOUTUBE_CHANNEL_ID` | Owner can approve and publish value-first official updates only; no links until compliance sign-off and approved metadata | Keep queue in draft until media rights and official byline are approved |


### 2026-06-04 Priority setup advancement

| Priority | Platform | Purpose | Status | Owner/manual step | Required assets | Secret/API variable or credential reference | Publish capability after connection | Next action |
| --- | --- | --- | --- | --- | --- | --- | --- |
| high | LinkedIn official company and founder pages | Build official founder/company credibility before inbound reviews, partnership talks, and builder channels | setup_packet_prepared_owner_action_required | OWNER_ACTION_REQUIRED: create/confirm official company page and founder profile, align mission fields, disclosure language, and canonical `https://nayovi.com` links. Confirm anti-promotion posting cadence and post-frequency policy. | `docs/seo-distribution/official-launch-profile-packet.md`, logo/avatar, legal-safe one-line mission, support URL, policy-safe headline copy | `SEO_AGENT_LINKEDIN_ACCESS_TOKEN`, `SEO_AGENT_LINKEDIN_ORGANIZATION_ID` | Owner can publish profile metadata updates and policy-first posts only after manual account verification and policy check | Keep drafts in `platform-drafts.md` as OWNER_REVIEW_REQUIRED until credentials and identity are approved |
| high | Google Search Console + Bing Webmaster | Prepare trusted crawl/index foundation for owned trust surfaces before external backlink claims | owner_action_required | OWNER_ACTION_REQUIRED: claim `nayovi.com`, `tachiyomiat.com`, and `translate-manhwa-ai.com`, add sitemap inventory, and approve only support-safe crawl statements | Domain ownership proofs, sitemap URL list, canonical link map, support/pricing pages | `SEO_AGENT_GOOGLE_SEARCH_CONSOLE_CREDENTIALS`, `SEO_AGENT_BING_WEBMASTER_API_KEY` | Owner can run crawl/index checks and use read-only claim blocks only after evidence is approved | Add official verification checkpoints to the owner action queue; keep all crawl claims in draft until approved |

## 2026-06-12 Cycle setup advancement

### 2026-06-12 Priority account setup packet

| Priority | Platform | Purpose | Status | Owner/manual step | Required assets | Secret/API variable or credential reference | Publish capability after connection | Next action |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| high | Google Search Console + Bing Webmaster | Establish crawl and index trust for owned trust properties (`nayovi.com`, `translate-manhwa-ai.com`) before using trust language in outreach materials | owner_action_required | OWNER_ACTION_REQUIRED: add/confirm DNS or file-based verification for both domains and publish stable sitemap index references in canonical profile pack | `docs/seo-distribution/webmaster-and-entity-verification-packet.md`, `docs/seo-distribution/public-evidence-index.md`, sitemap.xml URLs, policy + support pages | `SEO_AGENT_GOOGLE_SEARCH_CONSOLE_CREDENTIALS`, `SEO_AGENT_BING_WEBMASTER_API_KEY` | Owner can publish crawl-readiness language, index confidence notes, and partner-ready verification summary after credentials confirm healthy coverage | Run verification checks and then mark platform profile fields as OWNER_REVIEW_COMPLETE |
